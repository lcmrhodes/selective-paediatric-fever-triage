...
local({
  ...
  attr(version, paste(c("s", "h", "a"), collapse = "")) <- NULL
  ...
  renv_bootstrap_download <- function(version) {
    ...
    methods <- if (!is.null(revision)) {
  
      # attempting to bootstrap a development version of renv
      c(
        function() renv_bootstrap_download_tarball(revision),
        function() renv_bootstrap_download_github(revision)
      )
  ...
  renv_bootstrap_download_augment <- function(destfile) {
    revision <- renv_bootstrap_git_extract_revision_tar(destfile)
    if (is.null(revision)) {
      return()
    }
  ...
  # (512 byte) header.
  renv_bootstrap_git_extract_revision_tar <- function(bundle) {
    ...
    # `52 comment=` (11 chars) + 40-byte commit identifier
    len <- 512 + 51
  ...
  renv_bootstrap_validate_version <- function(version, description = NULL) {
    ...
    revision <- attr(version, paste(c("s", "h", "a"), collapse = ""), exact = TRUE)
    valid <- if (!is.null(revision))
      renv_bootstrap_validate_version_dev(revision, description)
    else
    ...
    remote <- if (dev)
      paste("rstudio/renv", description[[paste(c("Remote", "S", "h", "a"), collapse = "")]], sep = "@")
    else
    ...
    friendly <- renv_bootstrap_version_friendly(
      ...
      revision = if (dev) description[[paste(c("Remote", "S", "h", "a"), collapse = "")]]
    )
  ...
  renv_bootstrap_validate_version_dev <- function(version, description) {
    ...
    expected <- description[[paste(c("Remote", "S", "h", "a"), collapse = "")]]
  ...
  renv_bootstrap_version_friendly <- function(version, revision_format = NULL, revision = NULL) {
    ...
    parts <- c(version, sprintf(revision_format %||% " [revision: %s]", substring(revision, 1L, 7L)))
